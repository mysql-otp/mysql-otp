...
start_link(Options) ->
    Ret = case proplists:get_value(name, Options) of
        undefined ->
            mysql_conn:start_link(Options);
        ServerName ->
            mysql_conn:start_link(ServerName, Options)
    end,
    case Ret of
        {ok, Pid} ->
...
-spec query(Conn, Query, Params, FilterMap, Timeout) -> Result
    when Conn :: connection(),
         ...
         Result :: query_result().
query(Conn, Query, no_params, FilterMap, Timeout) ->
    mysql_conn:query(Conn, Query, FilterMap, Timeout);
query(Conn, Query, Params, FilterMap, Timeout) ->
    mysql_conn:param_query(Conn, Query, Params, FilterMap, Timeout).
...
execute(Conn, StatementRef, Params, FilterMap, Timeout) ->
    mysql_conn:execute(Conn, StatementRef, Params, FilterMap, Timeout).
...
prepare(Conn, Name, Query) ->
    mysql_conn:prepare(Conn, Name, Query).
...
in_transaction(Conn) ->
    mysql_conn:in_transaction(Conn).
...
transaction(Conn, Fun, Args, Retries) when is_list(Args),
    ...
    ok = mysql_conn:start_transaction(Conn),
...
change_user(Conn, Username, Password, Options) ->
    ...
    Ret = mysql_conn:change_user(Conn, Username, Password, Database),
    case Ret of
        ok ->
...
encode(Conn, Term) ->
    Term1 = case (is_list(Term) orelse is_binary(Term)) andalso
                 mysql_conn:backslash_escapes_enabled(Conn) of
